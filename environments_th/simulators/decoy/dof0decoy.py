...
class DOF0BallDecoy(BaseDecoy):
    ...
    def run(self, index: _SupportedIndexType | None = None):
        ...
        super().run()  # time++
        # 后处理
        self._pos_e[index, :] = pe
        self._vel_e[index, :] = ve
    ...
    def _propagate(self, index: _SupportedIndexType | None):
        ...
        if self.DEBUG:
            if is_new_timeout.view(-1, 1)[0, 0].item():
                logr.debug(
                    (
                        "obj: {}".format(self.__class__.__name__),
                        "status: {}".format(self.status.view(-1, 1)[0, 0].item()),
                        "new_timeout: {}".format(
                            is_new_timeout.view(-1, 1)[0, 0].item()
                        ),
                    )
                )
        return
    ...
    def _run_ode(
        ...
        pos_e_next, vel_e_next = ode_solver(_f, t_s, (pos_e, vel_e), dt_s)
        if self.DEBUG:
            logr.debug(
                (
                    "id:{}".format(self.acmi_id[0, 0].item()),
                    "tas:{:.3g}".format(vel_e_next[0].norm().item()),
                    "pos_e:{}".format(pos_e_next[0].cpu().numpy()),
                    "vel_e:{}".format(vel_e_next[0].cpu().numpy()),
                )
            )
