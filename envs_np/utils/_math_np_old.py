...
def Lx(angle_rad: torch.Tensor) -> torch.Tensor:
    ...
    return torch.cat(L_flat, axis=-1).reshape(angle_rad.shape[0:1] + (3, 3))
...
def _Qx(angle_rad: torch.Tensor) -> torch.Tensor:
    ...
    return torch.cat(Q_flat, axis=-1)
...
def _rpy2mat(rpy: torch.Tensor):
    psi, theta, phi = torch.split(rpy, [1, 1, 1], axis=-1)  # (...,1)
    ...
    c3 = torch.cos(psi)
    s3 = torch.sin(psi)
    rx = torch.cat([_1, _0, _0, _0, c1, -s1, _0, s1, c1], axis=-1).reshape(
        newshape
    )  # (...,3,3)
    ry = torch.cat([c2, _0, s2, _0, _1, _0, -s2, _0, c2], axis=-1).reshape(
        newshape
    )  # (...,3,3)
    rz = torch.cat([c3, -s3, _0, s3, c3, _0, _0, _0, _1], axis=-1).reshape(
        newshape
    )  # (...,3,3)
    return rz @ ry @ rx  # (...,3,3)
...
def quat2prodmat(q: torch.Tensor) -> torch.Tensor:
    ...
    return torch.stack(mat_flat, axis=-1).reshape(q0.shape + (4, 4))
...
def _quat_mul(p: torch.Tensor, q: torch.Tensor) -> torch.Tensor:
    ...
    r = torch.cat(
        [
            ...
            reP * imQ + reQ * imP + torch.cross(imP, imQ, axis=-1),
        ],
        -1,
    )
    return r
...
def _rpy2quat(rpy_rad: torch.Tensor) -> torch.Tensor:
    roll, pitch, yaw = torch.split(rpy_rad, [1, 1, 1], axis=-1)  # (...,1)
    r_hf = roll * 0.5  # (...,1)
    p_hf = pitch * 0.5
    y_hf = yaw * 0.5
    _0 = torch.zeros_like(r_hf)
    q1 = torch.cat([torch.cos(r_hf), _sin(r_hf), _0, _0], axis=-1)
    q2 = torch.cat([torch.cos(p_hf), _0, _sin(p_hf), _0], axis=-1)
    q3 = torch.cat([torch.cos(y_hf), _0, _0, _sin(y_hf)], axis=-1)
...
def _ned2aer(xyz: torch.Tensor) -> torch.Tensor:
    ...
    return torch.stack([azi, elev, slantRange], axis=-1)
...
def _aer2ned(aer: torch.Tensor) -> torch.Tensor:
    az, el, r = torch.chunk(aer, 3, axis=-1)  # (...,1)
...
def _herp(
    ...
    position = torch.cat([position_0, position_1], axis=-1)
    velocity = torch.cat([velocity_0, velocity_1], axis=-1)
...
def herp(
    ...
    position = torch.stack([n, e, d], axis=-1)
    return position
