# 250605 飞仿相关张量运算扩展(针对pytorch)
from __future__ import annotations
from typing import TYPE_CHECKING
from math import pi as _PI, e as _E, tau as _2PI

if TYPE_CHECKING:
    from typing_extensions import TypeAlias
from typing import Sequence, Callable, Union

# import torch

_bkbn = torch
_stack = _bkbn.stack
_cat = _bkbn.cat
_chunk = _bkbn.chunk
_split = _bkbn.split
_zeros_like = _bkbn.zeros_like
_ones_like = _bkbn.ones_like
_sin = _bkbn.sin
_cos = _bkbn.cos
_atan2 = _bkbn.atan2
_asin = _bkbn.asin
_where = _bkbn.where
_pow = _bkbn.pow
_sqrt = _bkbn.sqrt
_norm = _bkbn.norm
_clamp = _bkbn.clamp
_clip = _clamp
_reshape = _bkbn.reshape
NDArray = np.ndarray


def affcmb(w, a, b):
    """Affine combination of two tensors.

    $$
    (1-w) * a + w * b
    $$

    Args:
        w: Weights for the affine combination, shape=(..., 1).
        a: First scalar or tensor, shape=(..., dims).
        b: Second scalar or tensor, shape=(..., dims).

    Returns:
        Affine combination of a and b, shape=(..., dims).
    """
    return a + (b - a) * w


def affcmb_inv(y, a, b):
    """
    仿射组合的逆运算

    Args:
        y: a+w*(b-a), shape=(..., 1)
        a: 端点1, shape=(..., dims)
        b: 端点2, shape=(..., dims)

    Returns:
        w: 仿射系数
    """
    m = b - a
    # assert all(m!= 0)
    is0 = (m == 0) + 0.0
    return ((y - a) * is0) / (m + is0)


def B01toI(x: np.ndarray) -> np.ndarray:
    """B(0,1)=[-1,1]->I=[0,1]"""
    return (x + 1) * 0.5


def ItoB01(x: np.ndarray) -> np.ndarray:
    """I=[0,1]->B(0,1)=[-1,1]"""
    return x * 2 - 1


@np.jit.script
def _normalize(x: np.ndarray, eps: float = 1e-9) -> np.ndarray:
    return x / x.norm(dim=-1, p=2, keepdim=True).clip(eps)


@np.jit.script
def _quat_split_keepdim(q: np.ndarray):
    q0, q1, q2, q3 = q.split([1, 1, 1, 1], axis=-1)
    return q0, q1, q2, q3


@np.jit.script
def _quat_re(q: np.ndarray):
    return q.split([1, 3], axis=-1)[0]


@np.jit.script
def _quat_im(q: np.ndarray):
    return q.split([1, 3], axis=-1)[1]


@np.jit.script
def _quat_rect_re(q: np.ndarray) -> np.ndarray:
    """(解决双倍缠绕问题)返回实部非负的等价四元数"""
    reQ = _quat_re(q)
    return where(reQ < 0, -q, q)


@np.jit.script
def _an2quat(
    angle_rad: np.ndarray,
    axis: np.ndarray,
):
    assert (
        angle_rad.shape[:-1] == axis.shape[:-1]
    ), "Angle and axis must have the same shape except for the last dimension. "
    assert angle_rad.shape[-1] == 1, "Angle's last dimension must be 1."
    assert axis.shape[-1] == 3, "Axis must be a 3D vector."
    r = np.norm(axis, p=2, dim=-1, keepdim=True)
    axis = axis / r.clamp(min=1e-9)

    half_angle = angle_rad * 0.5
    q0 = _cos(half_angle)
    qv = _sin(half_angle) * axis
    q = np.cat([q0, qv], axis=-1)  # (...,4)
    q = _quat_rect_re(q)  # 确保实部非负
    return q


def an2quat(
    angle_rad: np.ndarray | float,
    axis: np.ndarray,
) -> np.ndarray:
    if not isinstance(angle_rad, np.ndarray):
        angle_rad = _zeros_like(axis[..., 0:1]) + angle_rad  # assert: 广播相容
    return _an2quat(angle_rad, axis)


# 定义基元旋转矩阵
def Lx(angle_rad: np.ndarray) -> np.ndarray:
    cos = np.cos(angle_rad)
    sin = np.sin(angle_rad)
    one = np.ones_like(cos)
    zero = np.zeros_like(cos)

    L_flat = (one, zero, zero, zero, cos, sin, zero, -sin, cos)
    return np.cat(L_flat, axis=-1).reshape(angle_rad.shape[0:1] + (3, 3))


def Ly(angle_rad: np.ndarray) -> np.ndarray:
    cos = np.cos(angle_rad)
    sin = np.sin(angle_rad)
    one = np.ones_like(cos)
    zero = np.zeros_like(cos)

    L_flat = (cos, zero, -sin, zero, one, zero, sin, zero, cos)
    return np.cat(L_flat, axis=-1).reshape(angle_rad.shape[0:1] + (3, 3))


def Lz(angle_rad: np.ndarray) -> np.ndarray:
    cos = np.cos(angle_rad)
    sin = np.sin(angle_rad)
    one = np.ones_like(cos)
    zero = np.zeros_like(cos)

    L_flat = (cos, sin, zero, -sin, cos, zero, zero, zero, one)
    return np.cat(L_flat, axis=-1).reshape(angle_rad.shape[0:1] + (3, 3))


@np.jit.script
def _Qx(angle_rad: np.ndarray) -> np.ndarray:
    """绕x轴旋转的规范四元数"""
    ah = angle_rad * 0.5
    cos = np.cos(ah)
    sin = np.sin(ah)
    zero = np.zeros_like(cos)

    Q_flat = (cos, sin, zero, zero)
    return np.cat(Q_flat, axis=-1)


@np.jit.script
def _Qy(angle_rad: np.ndarray) -> np.ndarray:
    """绕y轴旋转的规范四元数"""
    ah = angle_rad * 0.5
    cos = np.cos(ah)
    sin = np.sin(ah)
    zero = np.zeros_like(cos)

    Q_flat = (cos, zero, sin, zero)
    return np.cat(Q_flat, axis=-1)


@np.jit.script
def _Qz(angle_rad: np.ndarray) -> np.ndarray:
    """绕z轴旋转的规范四元数"""
    ah = angle_rad * 0.5
    cos = np.cos(ah)
    sin = np.sin(ah)
    zero = np.zeros_like(cos)

    Q_flat = (cos, zero, zero, sin)
    return np.cat(Q_flat, axis=-1)


def Qx(angle_rad: np.ndarray) -> np.ndarray:
    """绕x轴旋转的规范四元数"""
    assert angle_rad.shape[-1] == 1, "Angle's last dimension must be 1."
    return _Qx(angle_rad)


def Qy(angle_rad: np.ndarray) -> np.ndarray:
    """绕y轴旋转的规范四元数"""
    assert angle_rad.shape[-1] == 1, "Angle's last dimension must be 1."
    return _Qy(angle_rad)


def Qz(angle_rad: np.ndarray) -> np.ndarray:
    """绕z轴旋转的规范四元数"""
    assert angle_rad.shape[-1] == 1, "Angle's last dimension must be 1."
    return _Qz(angle_rad)


@np.jit.script  # 15%
def _rpy2mat(rpy: np.ndarray):
    psi, theta, phi = np.split(rpy, [1, 1, 1], axis=-1)  # (...,1)
    newshape = psi.shape[:-1] + (3, 3)
    _1 = np.ones_like(psi)
    _0 = np.zeros_like(psi)
    c1 = np.cos(phi)
    s1 = np.sin(phi)
    c2 = np.cos(theta)
    s2 = np.sin(theta)
    c3 = np.cos(psi)
    s3 = np.sin(psi)
    rx = np.cat([_1, _0, _0, _0, c1, -s1, _0, s1, c1], axis=-1).reshape(
        newshape
    )  # (...,3,3)
    ry = np.cat([c2, _0, s2, _0, _1, _0, -s2, _0, c2], axis=-1).reshape(
        newshape
    )  # (...,3,3)
    rz = np.cat([c3, -s3, _0, s3, c3, _0, _0, _0, _1], axis=-1).reshape(
        newshape
    )  # (...,3,3)
    return rz @ ry @ rx  # (...,3,3)


@np.jit.script  # 21%
def _rpy2mat_inv(Reb: np.ndarray, roll_ref_rad: np.ndarray):
    assert (
        roll_ref_rad.shape[-1] == 1
    ), "Roll reference angle's last dimension must be 1."
    assert (
        Reb.shape[:-2] == roll_ref_rad.shape[:-1]
    ), "Reb and roll_ref_rad must have the same shape except for the last dimension."
    s2 = np.clip(-Reb[..., 2, 0], -1.0, 1.0)
    pitch = np.arcsin(s2)
    c2s1 = Reb[..., 2, 1]  # R32
    c2c1 = Reb[..., 2, 2]  # R33
    c2s3 = Reb[..., 1, 0]  # R21
    c2c3 = Reb[..., 0, 0]  # R11

    eps = 1e-4  # 万向节死锁阈值(jit 警告: 别放在参数表里)
    gl = 1 - np.abs(s2) < eps
    #
    roll_ref_rad = roll_ref_rad.squeeze(-1)  # (...,)
    s1 = np.sin(roll_ref_rad)
    c1 = np.cos(roll_ref_rad)
    roll_ref_rad = np.arctan2(s1, c1)
    #
    s2s1 = s1 * s2
    s2c1 = c1 * s2
    R22 = Reb[..., 1, 1]
    R12 = Reb[..., 0, 1]
    R23 = Reb[..., 1, 2]
    R13 = Reb[..., 0, 2]
    s3 = s2s1 * R22 - c1 * R12 + s2c1 * R23 + s1 * R13
    c3 = c1 * R22 + s2s1 * R12 - s1 * R23 + s2c1 * R13

    roll = np.where(gl, roll_ref_rad, np.arctan2(c2s1, c2c1))
    yaw = np.where(gl, np.arctan2(s3, c3), np.arctan2(c2s3, c2c3))

    rpy = _stack([roll, pitch, yaw], axis=-1)
    return rpy


@np.jit.script
def _quat_norm(q: np.ndarray) -> np.ndarray:
    return q.norm(dim=-1, p=2, keepdim=True)


@np.jit.script
def _quat_normalize(q: np.ndarray) -> np.ndarray:
    return q / _quat_norm(q).clamp(min=1e-9)


def quat_norm(q: np.ndarray) -> np.ndarray:
    """Computes the norm of a quaternion.

    Args:
        q: The quaternion in (w, x, y, z). shape: (..., 4).

    Returns:
        The norm of the quaternion. shape: (..., 1).
    """
    return _quat_norm(q)


def quat_normalize(q: np.ndarray) -> np.ndarray:
    """Normalizes a quaternion to have unit length.

    Args:
        q: The quaternion in (w, x, y, z). shape: (..., 4).
    Returns:
        The normalized quaternion in (w, x, y, z). shape: (..., 4).
    """
    return _quat_normalize(q)


def normalize(x: np.ndarray, eps: float = 1e-9) -> np.ndarray:
    """Normalizes a given input tensor to unit length.

    Args:
        x: Input tensor of shape=(N, dims).
        eps: A small value to avoid division by zero. Defaults to 1e-9.

    Rets:
        Normalized tensor of shape=(N, dims).
    """
    return _normalize(x)


def is_normalized(v: np.ndarray, eps: float = 1e-6) -> np.ndarray:
    return ((v.norm(dim=-1, p=2, keepdim=True) - 1).abs() <= eps).all()


@np.jit.script
def _quat_conj(q: np.ndarray) -> np.ndarray:
    q0 = _quat_re(q)
    qv = _quat_im(q)
    return _cat((q0, -qv), axis=-1)


@np.jit.script
def _quat_inv(q: np.ndarray) -> np.ndarray:
    return _quat_conj(q) / (_quat_re(q) ** 2).clamp(min=1e-12)


def quat_split_keepdim(q: np.ndarray):
    """拆分为分量.

    Args:
        q: The quaternion in (w, x, y, z). shape: (..., 4).

    Returns:
        Re(q): 实部, shape: (..., 1).
        Im(q): 虚部, shape: (..., 3).
    """
    return _quat_split_keepdim(q)


def quat_re(q: np.ndarray) -> np.ndarray:
    """Get the real part of a quaternion.

    Args:
        q: The quaternion in (w, x, y, z). shape: (..., 4).

    Returns:
        The real part of the quaternion. shape: (..., 1).
    """
    return _quat_re(q)


def quat_im(q: np.ndarray) -> np.ndarray:
    """Get the imaginary part of a quaternion.

    Args:
        q: The quaternion in (w, x, y, z). shape: (..., 4).

    Returns:
        The imaginary part of the quaternion. shape: (..., 3).
    """
    return _quat_im(q)


def quat_conj(q: np.ndarray) -> np.ndarray:
    """Computes the conjugate of a quaternion.

    Args:
        q: The quaternion orientation in (w, x, y, z). shape: (..., 4).

    Rets:
        The conjugate quaternion in (w, x, y, z). shape: (..., 4).
    """
    return _quat_conj(q)


def quat2prodmat(q: np.ndarray) -> np.ndarray:
    r"""Computes the matrix of a quaternion.
    四元数左乘等效为矩阵乘法（线性算子作用）

    $$
    q \otimes p = M(q) p, \forall p \in\mathbb{H}
    $$

    Args:
        q: The quaternion orientation in (w, x, y, z). shape: (..., 4).

    Rets:
        The matrix of quaternion. shape: (..., 4, 4).
    """
    q0, q1, q2, q3 = q.unbind(dim=-1)
    nq1 = -q1
    nq2 = -q2
    nq3 = -q3
    mat_flat = (q0, nq1, nq2, nq3, q1, q0, nq3, q2, q2, q3, q0, nq1, q3, nq2, q1, q0)
    return np.stack(mat_flat, axis=-1).reshape(q0.shape + (4, 4))


def mati(q: np.ndarray) -> np.ndarray:
    """Computes the matrix of a quaternion.

    Args:
        q: The quaternion orientation in (w, x, y, z). shape: (..., 4).

    Rets:
        The matrix inverse of quaternion. shape: (..., 4, 4).
    """
    q0, q1, q2, q3 = q.unbind(dim=-1)

    mat_flat = (q0, -q1, -q2, -q3, q1, q0, q3, -q2, q2, -q3, q0, q1, q3, q2, -q1, q0)
    return np.stack(mat_flat, axis=-1).reshape(q0.shape + (4, 4))


@np.jit.script
def _quat2mat_sqrt(q: np.ndarray) -> np.ndarray:
    q0, q1, q2, q3 = q.split([1, 1, 1, 1], axis=-1)  # (...,1)
    nq0 = -q0
    nq1 = -q1
    nq2 = -q2
    nq3 = -q3
    m = _stack(
        [
            _cat([q0, nq1, nq2, nq3], axis=-1),
            _cat([nq1, nq0, q3, nq2], axis=-1),
            _cat([nq2, nq3, nq0, q1], axis=-1),
            _cat([nq3, q2, nq1, nq0], axis=-1),
        ],
        -2,
    )
    return m


def quat2mat_sqrt(q: np.ndarray) -> np.ndarray:
    r"""Computes the square root of the rotation matrix of a quaternion.

    $$
    S=J M(Q)\\
    S^2 h = Q\otimes h \otimes Q^*, \forall h \in \mathbb{H}
    $$

    Args:
        q: The quaternion orientation in (w, x, y, z). shape: (..., 4).

    Rets:
        The square root of the rotation matrix of the quaternion. shape: (..., 4, 4).
    """
    return _quat2mat_sqrt(q)


@np.jit.script
def _quat_mul(p: np.ndarray, q: np.ndarray) -> np.ndarray:
    # 旧版:仅支持 (B,4) 尺寸
    # r = np.bmm(quat2prodmat(p), q.unsqueeze(-1)).squeeze(-1)
    # return r

    # 新版:支持任意尺寸&计算耗时减少 25%
    reP = quat_re(p)
    imP = quat_im(p)
    reQ = quat_re(q)
    imQ = quat_im(q)
    r = np.cat(
        [
            reP * reQ - np.sum(imP * imQ, -1, keepdim=True),
            reP * imQ + reQ * imP + np.cross(imP, imQ, axis=-1),
        ],
        -1,
    )
    return r


@np.jit.script
def _rpy2quat(rpy_rad: np.ndarray) -> np.ndarray:
    roll, pitch, yaw = np.split(rpy_rad, [1, 1, 1], axis=-1)  # (...,1)
    r_hf = roll * 0.5  # (...,1)
    p_hf = pitch * 0.5
    y_hf = yaw * 0.5
    _0 = np.zeros_like(r_hf)
    q1 = np.cat([np.cos(r_hf), _sin(r_hf), _0, _0], axis=-1)
    q2 = np.cat([np.cos(p_hf), _0, _sin(p_hf), _0], axis=-1)
    q3 = np.cat([np.cos(y_hf), _0, _0, _sin(y_hf)], axis=-1)
    return _quat_mul(_quat_mul(q3, q2), q1)


def quat_mul(p: np.ndarray, q: np.ndarray) -> np.ndarray:
    r"""Multiply two quaternions together.

    $$
    Re(p \otimes q) = Re(p) * Re(q) - Im(p) \dot Im(q)
    Im(p \otimes q) = Re(p) * Im(q) + Re(q) * Im(p) + Im(p) \times Im(q)\\
    $$

    Args:
        p: The first quaternion in (w, x, y, z). shape: (..., 4).
        q: The second quaternion in (w, x, y, z). shape: (..., 4).

    Rets:
        The product of the two quaternions in (w, x, y, z). shape: (..., 4).
    """
    return _quat_mul(p, q)


def _crossmat_kern(v: np.ndarray) -> np.ndarray:
    x, y, z = v.unbind(dim=-1)  # (...,1)
    _0 = np.zeros_like(x)
    return _cat([_0, -z, y, z, _0, -x, -y, x, _0], axis=-1).reshape(
        v.shape[:-1] + (3, 3)
    )  # (...,3,3)


def crossmat(v: np.ndarray):
    r"""
    左叉积矩阵
    $v_\times a= v \times a$
    Args:
        v: The vector in (x, y, z). shape: (..., 3).
    Returns:
        The cross product matrix of vector v. shape: (..., 3, 3).
    """
    return _crossmat_kern(v)


@np.jit.script
def _quat2mat(q: np.ndarray) -> np.ndarray:
    # 旧版:只支持输入q为 (B,4) 二阶张量
    # rotation_matrix = np.bmm(mati(q), quat2prodmat(quat_conjugate(q)))
    # return rotation_matrix[..., 1:, 1:]

    # 新版:支持任意尺寸&计算耗时减少 20%
    # assert is_normalized(q), "Quaternion must be normalized."
    q0, q1, q2, q3 = quat_split_keepdim(q)  # (...,1)
    _2q0 = q0 + q0
    _2q1 = q1 + q1
    _2q2 = q2 + q2
    _2q3 = q3 + q3
    _2q0q1 = _2q0 * q1
    _2q0q2 = _2q0 * q2
    _2q0q3 = _2q0 * q3
    _2q1q2 = _2q1 * q2
    _2q1q3 = _2q1 * q3
    _2q2q3 = _2q2 * q3
    _2q0q0_1 = _2q0 * q0 - 1.0
    A11 = _2q1 * q1 + _2q0q0_1
    A22 = _2q2 * q2 + _2q0q0_1
    A33 = _2q3 * q3 + _2q0q0_1
    A12 = _2q1q2 - _2q0q3
    A21 = _2q1q2 + _2q0q3
    A13 = _2q1q3 + _2q0q2
    A31 = _2q1q3 - _2q0q2
    A23 = _2q2q3 - _2q0q1
    A32 = _2q2q3 + _2q0q1
    m = _stack(
        [
            _cat([A11, A12, A13], axis=-1),
            _cat([A21, A22, A23], axis=-1),
            _cat([A31, A32, A33], axis=-1),
        ],
        -2,
    )  # (...,3,3)
    return m


def quat2mat(q: np.ndarray) -> np.ndarray:
    r"""
    四元数->3D旋转矩阵
    $$
    R(q) Im(h) = |q| Im(q \otimes h \otimes q^*)
    $$
    Args:
        q: The quaternion in (w, x, y, z). shape: (..., 4).

    Rets:
        The rotation matrix of quaternion. shape: (..., 3, 3).
    """
    return _quat2mat(q)


@np.jit.script
def _quat_rot(q: np.ndarray, v: np.ndarray):
    # 旧版:仅支持 (B,4) 尺寸
    # rotation_matrix = quat2mat(q)
    # u = np.bmm(rotation_matrix, v.unsqueeze(-1)).squeeze(-1)
    # return u

    # 新版:支持任意尺寸&计算耗时减少 13%
    # assert is_normalized(q), "Quaternion must be normalized."
    reQ, imQ = q.split([1, 3], axis=-1)
    _2q0 = reQ + reQ
    _2qv = imQ + imQ
    u = (
        (_2q0 * reQ - 1.0) * v
        + _2qv * ((imQ * v).sum(-1, keepdim=True))
        + _2q0 * np.cross(imQ, v, axis=-1)
    )
    return u


def quat_rotate(q: np.ndarray, v: np.ndarray) -> np.ndarray:
    r"""Rotate a vector by a quaternion along the last dimension of q and v.

    $$
    Im(Q \otimes (0,v) \otimes Q^*)
    $$

    Args:
        q: Normalized quaternion in (w, x, y, z). shape: (..., 4).
        v: 3D vector in (x, y, z). shape: (..., 3).

    Returns:
        The rotated vector in (x, y, z). shape: (..., 3).
    """
    return _quat_rot(q, v)


def quat_rotate_inv(q: np.ndarray, v: np.ndarray) -> np.ndarray:
    r"""Inverse rotate a vector by a quaternion along the last dimension of q and v.

    $$
    Im(Q^* \otimes (0,v) \otimes Q)
    $$

    Args:
        q: Normalized quaternion in (w, x, y, z). shape: (..., 4).
        v: 3D vector in (x, y, z). shape: (..., 3).

    Returns:
        The rotated vector in (x, y, z). shape: (..., 3).
    """
    # rotation_matrix = quat2rotmat(quat_conjugate(q))
    # u = np.bmm(rotation_matrix, v.unsqueeze(-1)).squeeze(-1)

    # assert is_normalized(q), "Quaternion must be normalized."
    return quat_rotate(quat_conj(q), v)


def quat_enu_ned() -> np.ndarray:
    return quat_mul(
        Qz(np.tensor([[np.pi * 0.5]])),
        quat_mul(Qy(np.tensor([[0]])), Qx(np.tensor([[np.pi]]))),
    )


def rpy2mat(
    rpy: np.ndarray,
):
    r"""
    Z-Y-X 内旋矩阵
    $$
    R = R_z(\psi) R_y(\theta) R_x(\phi)
    $$
    Args:
        rpy (np.ndarray): (滚转\psi,俯仰\theta,偏航\phi),unit:rad, shape=(..., 3)
    Returns:
        np.ndarray: 旋转矩阵, shape=(..., 3, 3)
    """
    return _rpy2mat(rpy)


def rpy2mat_inv(Reb: np.ndarray, roll_ref_rad: np.ndarray | float = 0.0):
    """
    rpy2mat 的逆映射，当发生万向节死锁时，需要给出 roll_ref_rad
    Args:
        Reb (np.ndarray): 旋转矩阵, shape=(..., 3, 3)
        roll_ref_rad (np.ndarray | float): 滚转角参考值, 单位: rad, shape=(..., 1) 或标量
    """
    assert Reb.shape[-2:] == (3, 3), "expected matrix shape=(...,3,3), got {}".format(
        Reb.shape
    )
    if not isinstance(roll_ref_rad, np.ndarray):
        roll_ref_rad = roll_ref_rad + _zeros_like(Reb[..., 0, 0:1])  # assert: 广播相容
    return _rpy2mat_inv(Reb, roll_ref_rad)


def rpy2quat(rpy_rad: np.ndarray) -> np.ndarray:
    r"""
    输入欧拉角(rad)，输出规范四元数
    $$
    Q=Q_{e3,yaw}*Q_{e2,pitch}*Q_{e1,roll}
    $$
    Args:
        rpy_rad (np.ndarray): 输入欧拉角(rad), shape: (..., 3)
    Returns:
        np.ndarray: 规范四元数, shape: (..., 4)
    """
    return _rpy2quat(rpy_rad)


def rpy2quat_inv(q: np.ndarray, roll_ref_rad: np.ndarray | float = 0.0) -> np.ndarray:
    r"""四元数反解欧拉角
    $$
    q=Q_z(yaw)\otimes Q_y(pitch) \otimes Q_x(roll)
    $$

    Args:
        q (np.ndarray): 规范四元数 shape: (..., 4).
        roll_ref_rad (np.ndarray | float, optional): 当前滚转角(死锁时备用) shape: (..., 1) or scalar. Defaults to 0.0.

    Returns:
        np.ndarray: 欧拉角(滚转\phi,俯仰\theta,偏航\psi),单位:rad, shape=(..., 3)
    """
    Reb = _quat2mat(q)
    rpy = rpy2mat_inv(Reb, roll_ref_rad)
    return rpy


def euler_from_quat(
    q: np.ndarray, roll_ref_rad: np.ndarray | float = 0.0
) -> np.ndarray:
    # q0, q1, q2, q3 = q.unbind(dim=-1)
    # # 隐患: 万向节死锁求解

    # phi = np.arctan2(2 * (q2 * q3 + q0 * q1), 1 - 2 * (q1 * q1 + q2 * q2))
    # theta = np.arcsin(np.clamp(-2 * (q3 * q1 - q0 * q2), -1, 1))
    # psi = np.arctan2(2 * (q1 * q2 + q0 * q3), 1 - 2 * (q2 * q2 + q3 * q3))

    # return np.stack([phi, theta, psi], axis=-1)

    return rpy2quat_inv(q, roll_ref_rad)


from pymap3d import enu2aer


@np.jit.script
def _ned2aer(xyz: np.ndarray) -> np.ndarray:
    _R = np.norm(xyz, p=2, dim=-1, keepdim=True)
    x, y, z = _split(xyz, [1, 1, 1], axis=-1)  # (...,1)
    _is0 = _R < 1e-3  # 过零处理
    _1 = _ones_like(x)
    _0 = _zeros_like(x)
    x = where(_is0, _1, x)
    y = where(_is0, _0, y)
    z = where(_is0, _0, z)
    rxy2 = _pow(y, 2) + _pow(x, 2)
    rxy = _sqrt(rxy2)
    slantRange = np.sqrt(rxy2 + np.pow(z, 2))

    elev = _atan2(-z, rxy)  # -> [-pi/2,pi/2]
    azi = _atan2(x, y) % _2PI  # -> [0,2pi)
    return np.stack([azi, elev, slantRange], axis=-1)


def ned2aer(xyz: np.ndarray) -> np.ndarray:
    r"""求解 NED xyz 直角坐标对应的 方位角 azimuth, 俯仰角 elevation, 距离 r\
    即 (r,0,0) 依次 绕Z内旋 azimuth, 绕Y内旋 elevation 得到 (x,y,z), 右手定则

    Args:
        xyz (np.ndarray): NED 直角坐标 shape: (...,3)

    Returns:
        aer (np.ndarray): shape: (...,3)
    """
    return _ned2aer(xyz)


@np.jit.script
def _aer2ned(aer: np.ndarray) -> np.ndarray:
    az, el, r = np.chunk(aer, 3, axis=-1)  # (...,1)

    rxy = r * np.cos(el)
    z = -r * np.sin(el)
    x = rxy * np.cos(az)
    y = rxy * np.sin(az)
    return np.cat([x, y, z], axis=-1)


def aer2ned(aer: np.ndarray) -> np.ndarray:
    """
    ned2aer 的逆映射
    Args:
        aer (np.ndarray): 方位角 azimuth, 俯仰角 elevation, 距离 r\
            即 (r,0,0) 依次 绕Z内旋 azimuth, 绕Y内旋 elevation 得到 (x,y,z), 右手定则 shape: (...,3)

    Returns:
        ned (np.ndarray): NED 直角坐标 shape: (...,3)
    """
    return _aer2ned(aer)


@np.jit.script
def _uvw2alpha_beta(uvw: np.ndarray):
    uvw = normalize(uvw)
    u, v, w = _chunk(uvw, 3, axis=-1)  # (...,1)
    alpha = _atan2(w, u)
    beta = _asin(v)
    return alpha, beta


def uvw2alpha_beta(uvw: np.ndarray):
    r"""
    NED 体轴速度分量 (U,V,W)->(\alpha,\beta)\
    坐标系 旋转关系\
    $$
    \Phi_v R_z(-\beta) R_y(\alpha) = \Phi_b
    $$

    (U,V,W) 单位向量的分解

    $$
    i_{v/b} = ( \cos\beta \cos\alpha, 
                \sin\beta, 
                \cos\beta \sin\alpha)
    $$

    Args:
        uvw (np.ndarray): 体轴速度分量 shape: (...,3)

    Returns:
        alpha (np.ndarray): 迎角 \in [-pi/2,pi/2] shape: (...,1)
        beta (np.ndarray): 侧滑角 \in (-pi,pi] shape: (...,1)
    """
    # assert (norm(uvw)>0).all()
    return _uvw2alpha_beta(uvw)


@np.jit.script
def _vec_cosine(v1: np.ndarray, v2: np.ndarray, n1: np.ndarray, n2: np.ndarray):
    eps = 1e-6
    v1_is_zero = n1 < eps
    v2_is_zero = n2 < eps
    any_zero = v1_is_zero | v2_is_zero
    c = np.where(
        any_zero,
        np.zeros_like(n1),
        np.sum(v1 * v2, -1, keepdim=True) / (n1 * n2),
    )
    return c


def vec_cosine(
    v1: np.ndarray,
    v2: np.ndarray,
    n1: np.ndarray | float | None = None,
    n2: np.ndarray | float | None = None,
):
    """
    计算两个向量的余弦值
    $$
    cos(v1,v2)=\frac{v1\cdot v2}{|v1||v2|}
    $$
    Args:
        v1 (np.ndarray): 向量1 shape: (...,3)
        v2 (np.ndarray): 向量2 shape: (...,3)
        n1 (np.ndarray|float|None): 向量1的长度 shape: (...,1) or scalar
        n2 (np.ndarray|float|None): 向量2的长度 shape: (...,1) or scalar

    Returns:
        np.ndarray: 余弦值 shape: (...,1)
    """
    if not isinstance(n1, np.ndarray):
        n1 = np.norm(v1, p=2, dim=-1, keepdim=True)
    if not isinstance(n2, np.ndarray):
        n2 = np.norm(v2, p=2, dim=-1, keepdim=True)
    return _vec_cosine(v1, v2, n1, n2)


def _herp(
    position_0: np.ndarray,
    velocity_0: np.ndarray,
    position_1: np.ndarray,
    velocity_1: np.ndarray,
    t: np.ndarray,
) -> np.ndarray:
    position = np.cat([position_0, position_1], axis=-1)
    velocity = np.cat([velocity_0, velocity_1], axis=-1)
    dtype = position.dtype
    device = position.device
    x = np.cat(
        [
            np.zeros(size=(position.shape[0], 1), dtype=dtype, device=device),
            np.ones(size=(position.shape[0], 1), dtype=dtype, device=device),
        ],
        dim=-1,
    )

    r, c1 = x.shape
    _, c2 = t.shape

    x_prime_1 = x.repeat(1, c1).reshape(r, c1, c1).permute(0, 2, 1)
    x_prime_mask = np.eye(c1, device=device).repeat(r, 1, 1)
    x_prime_2 = x_prime_1 - x_prime_mask * x_prime_1
    x_prime = x_prime_2.unsqueeze(1).repeat(1, c2, 1, 1)

    t_prime_1 = t.unsqueeze(2).repeat(1, 1, c1)
    t_prime = t_prime_1.unsqueeze(3).repeat(1, 1, 1, c1)

    l_num_1 = t_prime - x_prime
    l_num_mask = np.eye(c1, device=device).repeat(r * c2, 1, 1).reshape(r, c2, c1, c1)
    l_num_2 = l_num_1 - l_num_mask * l_num_1 + l_num_mask
    l_num = np.prod(l_num_2, dim=2)

    l_den_1 = np.prod((x_prime_1.permute(0, 2, 1) - x_prime_1) + x_prime_mask, dim=1)
    l_den = l_den_1.unsqueeze(1).repeat(1, c2, 1)

    l = l_num / l_den

    l_prime_1 = 1.0 / ((x_prime_1.permute(0, 2, 1) - x_prime_1) + x_prime_mask)
    l_prime_2 = np.sum(l_prime_1, dim=1) - 1
    l_prime = l_prime_2.unsqueeze(1).repeat(1, c2, 1)

    # Create x_prime_3 for functions A and B
    x_prime_3 = x.unsqueeze(1).repeat(1, c2, 1)

    # Calculate function B
    B = (t_prime_1 - x_prime_3) * l * l

    # Calculate function A
    A = (1 - 2 * (t_prime_1 - x_prime_3) * l_prime) * l * l

    # Calculate final result H
    A_prime = position.unsqueeze(1).repeat(1, c2, 1) * A
    B_prime = velocity.unsqueeze(1).repeat(1, c2, 1) * B
    H = np.sum(A_prime, dim=2) + np.sum(B_prime, dim=2)

    return H


def herp(
    position_0: np.ndarray,
    velocity_0: np.ndarray,
    position_1: np.ndarray,
    velocity_1: np.ndarray,
    t: np.ndarray,
) -> np.ndarray:
    """
    args:
        position_0: shape: [n, 3]
        velocity_0: shape: [n, 3]
        position_1: shape: [n, 3]
        velocity_1: shape: [n, 3]
        t: shape: [n, m]

    rets:
        position: shape: [n, m, 3]
    """
    n = _herp(
        position_0[..., 0:1],
        velocity_0[..., 0:1],
        position_1[..., 0:1],
        velocity_1[..., 0:1],
        t,
    )  # n, shape: [n, m]
    e = _herp(
        position_0[..., 1:2],
        velocity_0[..., 1:2],
        position_1[..., 1:2],
        velocity_1[..., 1:2],
        t,
    )  # n, shape: [n, m]
    d = _herp(
        position_0[..., 2:3],
        velocity_0[..., 2:3],
        position_1[..., 2:3],
        velocity_1[..., 2:3],
        t,
    )  # n, shape: [n, m]

    position = np.stack([n, e, d], axis=-1)
    return position


def lerp(v_0: np.ndarray, v_1: np.ndarray, t: np.ndarray) -> np.ndarray:
    v_0 = v_0.unsqueeze(1).repeat(1, t.shape[-1], 1)
    v_1 = v_1.unsqueeze(1).repeat(1, t.shape[-1], 1)
    t = t.unsqueeze(-1).repeat(1, 1, v_0.shape[-1])

    v_t = (1 - t) * v_0 + t * v_1
    return v_t


def nlerp(q_0: np.ndarray, q_1: np.ndarray, t: np.ndarray) -> np.ndarray:
    """
    args:
        q_0: shape: [n, 4]
        q_1: shape: [n, 4]
        t: shape: [n, m]

    rets:
        q_t: shape: [n, m, 4]
    """

    # 钝角检测, 解决双倍覆盖问题
    dot = np.einsum("ij,ij->i", [q_0, q_1])
    indices = np.where(dot < 0)[0]
    q_1[indices] *= -1

    q_t = lerp(q_0, q_1, t)

    q_t = normalize(q_t)  # TODO:注意可能会导致问题，有待验证
    return q_t


_DynamicsFuncType: TypeAlias = Union[
    Callable[
        [
            np.ndarray | float,  # $0 时间
            Sequence[np.ndarray | float],  # $1 状态
        ],
        Sequence[np.ndarray | float],
    ],
    Callable,
]


def ode_rk45(
    f: _DynamicsFuncType,
    x0: Sequence[np.ndarray],
    t0: np.ndarray | float,
    dt: np.ndarray | float,
) -> list[np.ndarray]:
    """定步长 4阶 Runge-Kutta 法"""
    dt_hf = dt * 0.5
    t1 = t0 + dt_hf

    w1 = f(t0, x0)
    w2 = f(t1, [xi + dt_hf * wi for xi, wi in zip(x0, w1)])
    w3 = f(t1, [xi + dt_hf * wi for xi, wi in zip(x0, w2)])
    w4 = f(t0 + dt, [xi + dt * wi for xi, wi in zip(x0, w3)])

    x_next = [
        xi + dt / 6.0 * (wi1 + 2.0 * wi2 + 2.0 * wi3 + wi4)
        for xi, wi1, wi2, wi3, wi4 in zip(x0, w1, w2, w3, w4)
    ]
    return x_next


def ode_rk23(
    f: _DynamicsFuncType,
    x0: Sequence[np.ndarray],
    t0: np.ndarray | float,
    dt: np.ndarray | float,
) -> list[np.ndarray]:
    """定步长 2阶 Runge-Kutta 法（改进欧拉法）"""
    dt_hf = dt * 0.5
    t1 = t0 + dt_hf

    w1 = f(t0, x0)
    w2 = f(t1, [xi + dt_hf * wi for xi, wi in zip(x0, w1)])
    x_next = [xi + dt_hf * (wi1 + wi2) for xi, wi1, wi2 in zip(x0, w1, w2)]
    return x_next


def ode_euler(
    f: _DynamicsFuncType,
    x0: Sequence[np.ndarray],
    t0: np.ndarray | float,
    dt: np.ndarray | float,
):
    """
    欧拉法
    """
    w1 = f(t0, x0)
    x_next = [xi + dt * wi for xi, wi in zip(x0, w1)]
    return x_next


def modin(x: np.ndarray, a: np.ndarray | float, m: np.ndarray | float):
    r"""
    a+((x-a) mod m)
    if m=0, return a
    if m>0, y $\in [a,a+m)$
    if m<0, y $\in (a-m,a]$
    """
    if not (isinstance(a, np.ndarray) and isinstance(m, np.ndarray)):
        _0 = np.zeros_like(x)
        if not isinstance(a, np.ndarray):
            a = _0 + a  # assert: 广播相容
        if not isinstance(m, np.ndarray):
            m = _0 + m  # assert: 广播相容
    y = np.where(m == 0, a, (x - a) % m + a)
    return y


def delta_reg(a: np.ndarray, b: np.ndarray, r: np.ndarray | float = _PI):
    r"""
    计算 a-b 在 R=(-r,r] 上的最小幅度值

    $$
    \argmin_{ d\in R=(-r,r]: d=a-b (mod |R|) } |d|
    $$
    """
    # assert np.all(r > 0), "r must be positive."
    diam = r + r
    d = modin(a - b, 0, diam)  # in [0,2r)
    d = np.where(d <= r, d, d - diam)  # in (-r,r]
    return d


def delta_deg_reg(a: np.ndarray, b: np.ndarray):
    r"""
    $$
    \argmin_{ d\in R=(-180,180]: d=a-b (mod |R|) } |d|
    $$"""
    return delta_reg(a, b, 180)


def delta_rad_reg(a: np.ndarray, b: np.ndarray):
    r"""
    $$
    \argmin_{ d\in R=(-pi,pi]: d=a-b (mod |R|) } |d|
    $$"""
    return delta_reg(a, b, _PI)


def calc_zem(
    p1: NDArray,
    v1: NDArray,
    p2: NDArray,
    v2: NDArray,
    tmin=0,
    tmax=None,
) -> tuple[NDArray, NDArray]:
    r"""
    零控脱靶量

    $$
    \min_\{|(p_1+t v_1)-(p_2+t v_2)| | 0\leq t\leq t_\max\}
    $$

    Args:
        p1: shape=(n|1,d) 群体1的初始位置(t=0)
        v1: shape=(n|1,d) 群体1的速度
        p2: shape=(m|1,d) 群体2的初始位置(t=0)
        v2: shape=(m|1,d) 群体2的速度
        tmin: 最小时间步, 默认为0
        tmax: 最大时间步, 默认为None, 即 $\infty$
    Returns:
        rst: [MD, t_miss]

            MD: 脱靶量 shape=(n,m,1)
                $MD[i,j]:=min_{t\geq 0,t\leq tmax}\|(p1+t*v1)-(p2+t*v2)\|_2$

            t_miss: 脱靶时间 shape=(n,m,1)
                $t_{miss}[i,j]:=\min\argmin_{t\geq 0}\|(p1+t*v1)-(p2+t*v2)\|_2$
    """
    d = p1.shape[-1]
    p1 = _reshape(p1, (-1, 1, d))
    v1 = _reshape(v1, (-1, 1, d))
    p2 = _reshape(p2, (1, -1, d))
    v2 = _reshape(v2, (1, -1, d))
    dp = p1 - p2
    dv = v1 - v2
    pv = (dp * dv).sum(-1, True)  # (n,m,1)
    vv = (dv * dv).sum(-1, True)  # (n,m,1)
    _zeroV = vv <= 1e-6  # 过零处理
    _0 = 0 + vv
    t_miss = where(_zeroV, _0, -pv / (vv + _zeroV))  # (n,m,1)
    t_miss = _clip(t_miss, tmin, tmax)  # 投影时间
    md = _norm(dp + dv * t_miss, 2, -1, True)  # (n,m,1)
    return md, t_miss


def _demo():  # 自测
    from timeit import timeit

    # init seed
    seed = 42
    np.manual_seed(seed)
    np.cuda.manual_seed(seed)
    np.cuda.manual_seed_all(seed)
    np.backends.cudnn.deterministic = True
    np.backends.cudnn.benchmark = False

    bsz = (8,)

    def f():
        rpy = affcmb(np.rand([*bsz, 3], dtype=np.float32), np.pi, 2 * np.pi)
        roll = rpy[..., 0:1]
        reb = rpy2mat(rpy)
        rpy2 = rpy2mat_inv(reb, roll)
        return
        p = quat_normalize(np.rand([*bsz, 4], dtype=np.float32))
        q = quat_normalize(np.rand([*bsz, 4], dtype=np.float32))
        r = quat_mul(p, q)
        return

        qconj = quat_conj(q)
        assert is_normalized(q), "Quaternion must be normalized."
        v = normalize(np.rand([*bsz, 3], dtype=np.float32))  # 单位球面测试

        u = quat_rotate(q, v)
        v2 = quat_rotate(qconj, u)
        # v2 = quat_rotate_inverse(q, u)
        rer = (v - v2).abs().max()
        # print("Max error:", err.item())
        assert rer < 1e-4, "Quaternion rotation error is too large."
        # Rq = quat2rotmat(q)
        # v = Rq @ v.unsqueeze(-1)
        # m = _quat2rotmat_sqrt(q)

    print("Testing...")
    t = timeit(f, number=10000)
    print(t)
    # print(r, r.shape)
    # qvq = quat_rotate(q, v)
    # v2 = quat_rotate_inverse(q, qvq)
    # print(qvq, qvq.shape)


if __name__ == "__main__":
    _demo()
