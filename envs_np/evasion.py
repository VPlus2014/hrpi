...
# if TYPE_CHECKING:
#     from np.utils.tensorboard.writer import SummaryWriter
...
#     vec_cosine,
# )
# from environments_th.utils.tacview_render import ObjectState, AircraftAttr, MissileAttr
# from environments_th.reward_fns import (
#     TimeRewardFn,
#     LowAltitudeRewardFn,
#     LowAirSpeedRewardFn,
# )
# from environments_th.reward_fns.evasion import *
# from environments_th.termination_fns import *
# from environments_th.termination_fns.evasion import *
...
#         self.position_min_limit = np.tensor(position_min_limit, dtype=np.int64)
#         self.position_max_limit = np.tensor(position_max_limit, dtype=np.int64)
#         self.writer = writer
...
#             position_e=np.cat(
#                 [
#                     np.zeros(size=(self.num_envs, 2)),
#                     -6000 * np.ones(size=(self.num_envs, 1)),
#                 ],
#                 dim=-1,
#             ),
#             tas=600 * np.ones(size=(self.num_envs, 1)),
#             sim_step_size_ms=sim_step_size_ms,
#             device=self.device,
#         )

#         # 创建导弹模型（追击者）
#         random_vals = np.rand(self.num_envs, 3)
...
#         self.__sim_time_ms = np.zeros(size=(self.num_envs, 1), device=self.device)
...
#     @property
#     def sim_time_ms(self) -> np.ndarray:
...
#     def _cast_out(self, data: np.ndarray) -> np.ndarray | np.ndarray:
#         if self._out_as_np:
#             data = data.cpu().numpy()
#         return data
...
#         random_vals = np.rand(len(env_indices), 3)
#         positions = self.position_min_limit + random_vals * (
#             self.position_max_limit - self.position_min_limit
#         )
...
#     def step(self, action: np.ndarray, global_step: int):
...
#         rew = self.__get_rew()
#         truncated = np.zeros(
#             size=(self.num_envs, 1), dtype=np.bool, device=self.device
#         )
...
#     def _png(
#         self, target_position_e: np.ndarray, target_velocity_e: np.ndarray
#     ) -> np.ndarray:
...
#         omega = np.cross(dp, dv, axis=-1) / dem
#         ad_g = self.missile._N * np.cross(
#             dv, omega, dim=-1
...
#     def __get_obs(
#         self, env_indices: Sequence[int] | np.ndarray | None = None
#     ) -> OrderedDict:
#         if env_indices is None:
#             env_indices = np.arange(self.num_envs, device=self.device)
...
#             env_indices = np.tensor(env_indices, device=self.device)

#         elif isinstance(env_indices, np.ndarray):
#             # check indices
...
#         reward = np.zeros(size=(self.num_envs, 1), device=self.device)
#         for reward_fn in self._reward_fns:
...
#             terminated = np.logical_or(terminated, _terminated)
#         return terminated
