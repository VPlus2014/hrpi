...
import codes.utils
from codes.utils.tacview import TacviewRecorder, ACMI_Types, format_id
from codes.utils import math_np as mext

calc_zem1 = mext.calc_zem1
rpy2mat = mext.rpy2mat
norm = mext.norm

from codes.envs_np.simulators.decoy.base import BaseDecoy
from codes.envs_np.simulators.proto4model import BaseModelGroup
from codes.envs_np.simulators.missile import BaseMissile
from codes.envs_np.simulators.plane.p6dof import PlaneP6DOF as Plane_
from codes.envs_np.simulators.missile.p6dof import (
    MissileP6DOF as Missile,
)
from codes.envs_np.simulators.decoy.dof0 import DecoyDOF0Ball as Decoy
from codes.utils import log_ext
from codes.utils.time_ext import Timer_Pulse
...
def missile_regen(
    ...
    hmax: float,
    alt0: float,
    unit_tc: Callable[[BaseModelGroup, int], List[Sequence[str]]],
    ntry_tol=1000,
    ...
    for ntry in range(ntry_tol):
        ...
        if parent_pos is None:  # 无载机位置
            ...
            tas = np.clip(
                mext.norm(ego_vel_, 2, -1, keepdims=True), grp._Vmin, grp._Vmax
            )
...
def merge_info4observe(
    ego: BaseModelGroup,
    groups: List[BaseModelGroup],
    use_visual_block=False,
):
    m = ego.batch_size
    ego_alv = mext.unsqueeze(ego.is_alive(), -3)  # (...,1,m,1)
    ego_pos = mext.unsqueeze(ego.position_e(), -3)  # (...,1,m,d)
    ego_id = mext.unsqueeze(ego.acmi_id, -3)  # (...,1,m,1)
    ego_clr = mext.unsqueeze(ego.acmi_color, -3)  # (...,1,m,1)
    tmp_masks = []
    tmp_pos = []
    tmp_vel = []
    tmp_r = []
    ...
    for grp in groups:
        ...
        grp_alv = mext.unsqueeze(grp.is_alive(), -2)  # (...,n,1,1)
        ...
        if use_visual_block:
            tmp_r.append(mext.unsqueeze(grp.vis_radius(), -2))  # (...,n,1,1)
        tmp_idtt.append(not_self)
        tmp_id.append(grp_ids)

    mask = mext.cat(tmp_masks, axis=-3)  # (...,N,m,1)
    N = mask.shape[0]
    id2 = mext.cat(tmp_id, axis=-2)  # (...,N,1)
    pos2 = mext.cat(tmp_pos, axis=-2)  # (...,N,3)
    vel2 = mext.cat(tmp_vel, axis=-2)  # (...,N,3)
    if use_visual_block:
        visr2 = mext.cat(tmp_r, axis=-3)  # (...,N,1,1)
        not_self = mext.cat(tmp_idtt, axis=-3)  # (...N,m,1)
        visr12 = np.broadcast_to(visr2, mask.shape)  # (...,N,m,1)
        los12 = mext.unsqueeze(pos2, -2) - ego_pos  # (...,N,m,3)
        ...
        valid = mext.los_is_visible(
            mext.flatten(los12, -3, -2),
            mext.flatten(visr12, -3, -2),
            mext.flatten(mask, -3, -2),
        )[
            0
        ]  # (...,N*m,1)
        mask = mext.unflatten(valid, -2, (N, m))  # (...,N,m,1)
...
def try_kill_groups(
    ...
    alv1 = mext.unsqueeze(ego.is_dying(), -2)  # (...,m,1,1)
    ids1 = mext.unsqueeze(ego.acmi_id, -2)  # (...,m,1,1)
    p1 = mext.unsqueeze(ego.position_e(), -2)  # (...,m,1,d)
    kr = mext.unsqueeze((np.zeros_like(ego.tas()) + ego.kill_radius), -2)  # (...,m,1,1)
    #
    ...
    for grp in groups:
        ...
        ids2 = mext.unsqueeze(grp.acmi_id, -3)  # (...,1,n,1)
        alv2 = mext.unsqueeze(grp.is_alive(), -3)  # (...,1,n,1)
        p2 = mext.unsqueeze(grp.position_e(), -3)  # (...,1,n,d)
        is_self = ids1 == ids2  # (...,m,n,1)
        dij = mext.norm_p(p1 - p2, dim=-1, keepdim=True)  # (...,m,n,1)
...
def game_run(
    ...
    unit_groups: List[BaseModelGroup] = [
        agent,  # @group
        fri_msl,  # @group
        enm_msl,  # @group
        decoys,  # @group
            ...
            if ss == ss_reset:  # @reset
                ...
                if tacview_full:
                    ...
                    if not rst:
                        ...
                        #     pass
                        continue
                acmi_fn = acmi_dir / f"{sim_epi}.acmi"
...
def demo4LOSblock(
    ...
    t0 = time.time()
    rst = mext.los_is_visible(np.asarray(ps), np.asarray(brs))
    noblock = rst[0]  # (n,1)
    etc = rst[1]  # (n,1)
    blocked = ~noblock
