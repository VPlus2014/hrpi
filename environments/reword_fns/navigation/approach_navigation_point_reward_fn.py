...
class ApproachNavigationPointRewardFn(BaseRewardFn):
    ...
    def reset(self, env: NavigationEnv, env_indices=None):
        los = env.cur_nav_LOS
    ...
    def __call__(self, env: NavigationEnv, **kwargs) -> torch.Tensor:
        ...
        if _version == 1:
            ...
            # 时间积分是 |LOS(t_f)|-|LOS(t_0)|
            reward = -dR
            # return self.weight * v_proj.unsqueeze(-1) / 340
        elif _version == 2:  # 势差奖励
            reward = self.pre_distance - dij  # 对应积分 |LOS(t_f)|-|LOS(t_0)|
        elif _version == 3:  # 二次代价
            reward = -dij * dij  # 对应积分 \int_{t_0}^{t_f} |LOS(t)|^2
        elif _version == 4:  # L1代价
            reward = -dij  # 对应积分 \int_{t_0}^{t_f} |LOS(t)|
        else:
