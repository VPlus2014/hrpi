...
from environments.models.missile import BaseMissile, PointMassMissile
...
from environments.reword_fns import TimeRewardFn, LowAltitudeRewardFn, LowAirSpeedRewardFn
...
class EvasionEnv(gymnasium.Env):
    ...
    def __init__(
        ...
        positions = self.position_min_limit + random_vals * (self.position_max_limit - self.position_min_limit)
        self.missile = PointMassMissile(
            model_name = "missile",
            model_color = "Blue",
            position_e = positions,
        ...
        self.observation_space_dict["aircraft_velocity_e"] = spaces.Box(low=-1500, high=1500, shape=(3,), dtype=np.float32)
        self.observation_space_dict["aircraft_tas"] = spaces.Box(low=0, high=2000, shape=(1,), dtype=np.float32)
        self.observation_space_dict["aircraft_chi"] = spaces.Box(low=-np.pi, high=np.pi, shape=(1,), dtype=np.float32)
        self.observation_space_dict["aircraft_gamma"] = spaces.Box(low=-np.pi/2, high=np.pi/2, shape=(1,), dtype=np.float32)
        self.observation_space_dict["aircraft_mu"] = spaces.Box(low=-np.pi, high=np.pi, shape=(1,), dtype=np.float32)
        self.observation_space_dict["aircraft_alpha"] = spaces.Box(low=-np.pi/2, high=np.pi/2, shape=(1,), dtype=np.float32)
        ...
        self.observation_space_dict["missile_velocity_e"] = spaces.Box(low=-1500, high=1500, shape=(3,), dtype=np.float32)
        self.observation_space = space2box(self.observation_space_dict)

        #=======define action space=======#
        self.action_space_dict = spaces.Dict()
        self.action_space_dict["thrust_cmd"] = spaces.Box(low=0, high=1, shape=(1,), dtype=np.float32)
        self.action_space_dict["alpha_cmd"] = spaces.Box(low=-np.pi/4, high=np.pi/4, shape=(1,), dtype=np.float32)
        self.action_space_dict["mu_cmd"] = spaces.Box(low=-np.pi, high=np.pi, shape=(1,), dtype=np.float32)
        self.action_space = space2box(self.action_space_dict)
    ...
    def reset(self, env_indices: Sequence[int] | torch.Tensor | None = None):
        ...
        positions = self.position_min_limit + random_vals * (self.position_max_limit - self.position_min_limit)
        self.missile._position_g[env_indices] = positions.to(device=self.device)

        aer = ned2aer(self.missile.target.position_g[env_indices]-self.missile.position_g[env_indices])
        self.missile.set_q_kg(
            gamma = aer[..., 1:2], 
            chi = aer[..., 0:1],
        ...
        if 0 in env_indices:
            ...
            if self.render_mode == "tacview":
                ...
                file_path = self.render_dir / "{:07d}.acmi".format(self.__render_count+1)
    ...
    def step(self, action: torch.Tensor, global_step: int):
        ...
        for i in range(step_num):
            ...
            if self.render_mode is not None:
                if self.sim_time_ms[0].item()-self.__render_timestamp_ms > self.__render_interval_ms:
            ...
            n_cmd = self._png(target_position_e=self.aircraft.position_g, target_velocity_e=self.aircraft.velocity_g)
        ...
        if torch.any(done):
            ...
            self.writer.add_scalar("miss_distance/mean", miss_distance.mean().item(), global_step)
            if len(indices) > 1:
                self.writer.add_scalar("miss_distance/std", miss_distance.std().item(), global_step)
    ...
    def _png(self, target_position_e: torch.Tensor, target_velocity_e: torch.Tensor) -> torch.Tensor:
        dv = target_velocity_e - self.missile.velocity_g
        dp = target_position_e - self.missile.position_g
        dem = torch.einsum("ij,ij->i", [dp, dp]).unsqueeze(-1)
        omega = torch.cross(dp, dv, dim=-1)/dem
        a_e = self.missile._N*torch.cross(dv, omega, dim=-1)
        a_f = quat_rotate(self.missile.q_kg, a_e)

        return torch.clip(a_f[..., 1:]/self.missile._g, min=-self.missile._nyz_max, max=self.missile._nyz_max)
    ...
    def __record(self):
        ...
        # missile
        euler = euler_from_quat(self.missile.q_kg[0])
        euler[0] = 0.0
