...
class BaseMissile(BaseModel):
    ...
    def __init__(
        self,
        kill_radius: float = 10.0,  # 杀伤半径 unit: m
        ...
        **kwargs,  # other parameters
        ...
        self.demage = 100.0
        self.kill_radius = 20.0  # 毁伤半径 unit: m
    ...
    def is_launch(self, env_indices: _SupportedIndexType = None) -> torch.Tensor:
        return self.is_alive(env_indices)
    ...
    def observe(self, pos_e: torch.Tensor, vel_e: torch.Tensor, mask: torch.BoolTensor):
        ...
        Args:
            ...
            mask (torch.Tensor): 掩码, shape: (N,B), 物体 i 信息对导弹 j 是否可用
        """
    ...
    def _update_distance(
        ...
        """测量距离&更新脱靶量
        Args:
            new_distance (torch.Tensor): 最新测量距离, shape: (B,1)
        """
