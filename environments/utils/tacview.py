...
class TacviewRecorder:
    ...
    def demo():
        ...
        def rpy2mat(rpy):
            psi, theta, phi = np.split(rpy, 3, -1)  # (...,1)
            ...
            c3 = np.cos(psi)
            s3 = np.sin(psi)
            rx = np.concatenate([_1, _0, _0, _0, c1, -s1, _0, s1, c1], -1).reshape(
                newshape
            )  # (...,3,3)
            ry = np.concatenate([c2, _0, s2, _0, _1, _0, -s2, _0, c2], -1).reshape(
                newshape
            )  # (...,3,3)
            rz = np.concatenate([c3, -s3, _0, s3, c3, _0, _0, _0, _1], -1).reshape(
                newshape
            )  # (...,3,3)
            return rz @ ry @ rx  # (...,3,3)
