...
from ...utils.math_pt import affcmb, ode_rk23, ode_rk45, ode_euler, quat_conj, modin
...
class P6DOFMissile(BaseMissile):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._ic_tas = torch.zeros(_shape + [1], device=device, dtype=dtype)
        self._ic_rpy_ew = torch.zeros(_shape + [3], device=device, dtype=dtype)
        self._ic_pos_e = torch.zeros(_shape + [3], device=device, dtype=dtype)
    ...
    def reset(self, index: _SupportedIndexType | None):
        ...
        self._mass[index] = self._m0

        self._tas[index] = self._ic_tas[index]
        self._ppgt_tas2Vw(index)

        self._rpy_ew[index, :] = self._ic_rpy_ew[index, :]
        self._ppgt_rpy_ew2Qew(index)

        self._pos_e[index, :] = self._ic_pos_e[index, :]

        self._propagate(index)

        self._ppgt_ned2blh(index)
    ...
    def run(self, batch_index: _SupportedIndexType | None):
        msk = self.proc_index(batch_index)
        t = self.sim_time_s()
        ...
        self.set_mu(modin(mu_next, -math.pi, math.pi), msk)
        self._mass[...] = mass_next
    ...
    def _update_los(
        ...
        Args:
            ...
            unit_id (torch.Tensor): 全体单位ID, shape: (...,n,1)
            mask (torch.Tensor): bool 掩码, shape: (...,n,N), 对象[i] 信息对导弹 j 是否可用
        """
        assert (
            len(unit_pos.shape)
            == len(unit_vel.shape)
        ...
        unit_id = unit_id.unsqueeze(-3)  # (...,1,n,1)
        unit_id = unit_id + torch.zeros_like(self.acmi_id).unsqueeze(-2)  # (...,N,n,1)
        self._all_tgt_id = unit_id  # (...,N,1,1)

        unit_pos = unit_pos.unsqueeze(-3)  # (...,1,n,d)
        unit_vel = unit_vel.unsqueeze(-3)
        ego_pos = self._pos_e.unsqueeze(-2)  # (...,N,1,d)
        ego_vel = self._vel_e.unsqueeze(-2)
        los = unit_pos - ego_pos  # (...,N,n,3)
        dlos = unit_vel - ego_vel
        self._all_los_e = los  # (...,N,n,3)
        self._all_dlos_e = dlos
        self._all_los_mask = mask.swapaxes(-2, -1).unsqueeze(-1)  # (...,N,n,1)
    ...
    def _los2info(self):
        ...
        iw_e = quat_rotate(self._Q_ew, self._E1F).unsqueeze(-2)  # (...,N,1,d)
        ...
        self._ndet = self._candet.sum(-2, True)  # 大市场的目标数 (...,N,1,1)
        ...
        ener_V = all_dlos.pow(2).sum(-1, keepdim=True)  # (N,n,1)

        w2 = (w1 * ener_V).to(dtype=dtype)  # (N,n,1)
        w2sum = w2.sum(-2, keepdim=True)  # (N,1,1)
        w2_ = torch.where(anydet, w2 / w2sum, 0)  # (N,n,1)
        ...
        tgt_los = torch.where(_anytrk, tgt_los, fake_los)  # (N,3)
        tgt_dlos = torch.where(_anytrk, tgt_dlos, fake_vel)  # (N,3)
        ...
        dij = tgt_los.norm(dim=-1, keepdim=True)  # (N,1)
    ...
    def set_target(
        ...
        target_vel_e: torch.Tensor,
        target_id: torch.Tensor | None,
        dst_index: _SupportedIndexType | None,
        update_distance: bool = True,
    ):
        """
        (简化情形)直接传入目标
        Args:
            target_pos_e (torch.Tensor): 目标位置(NED地轴坐标), shape: (N,3)
            target_vel_e (torch.Tensor): 目标速度(NED地轴坐标), shape: (N,3)
            target_id (torch.Tensor|None): 目标ID, shape: (N,1)
            dst_index (_SupportedIndexType|None): 分配索引, 默认为所有, 长度=N
        """
        msk = self.proc_index(dst_index)
        self.target_pos_e[msk] = target_pos_e
        self.target_vel_e[msk] = target_vel_e
        if target_id is not None:
            self.target_id[msk] = target_id
            if self.DEBUG:
                self.logr.debug(("target_id<-{}".format(self.target_id[0, 0].item()),))

        if update_distance:
            los = target_pos_e - self._pos_e[msk]
            dij = los.norm(dim=-1, keepdim=True)
            self._update_distance(dij, msk)

    def _calc_mass(
        self, t_s: torch.Tensor, index: _SupportedIndexType | None
    ) -> torch.Tensor:
        index = self.proc_index(index)
        return self._m0 - torch.clip(t_s, 0, self._t_thrust_s) * self._dm
    ...
    def set_action(
        ...
        Args:
            value (torch.Tensor): 控制量, shape: (N,2)
                ny_cmd 侧向过载, unit: G
        ...
        ny_cmd, nz_cmd = torch.chunk(value, 2, -1)
        # 过载限制
        ...
        if self.DEBUG:
            logr.debug(
                (
                    "ny_d:{:.3g}".format(ny_cmd[0].item()),
                    "nz_d:{:.3g}".format(nz_cmd[0].item()),
                )
            )
    ...
    def _run_ode(
        ...
        ny, nz = torch.chunk(action, 2, -1)  # (...,1)

        f = lambda t, X: self._f(t, X, ny, nz)
        dt_s = dt_s * self.is_launch()
        pos_e_next, tas_next, Qew_next, mu_next, mass_next = ode_solver(
            f, t_s, (pos_e, tas, Qew, mu, mass), dt_s
        )
        if self.DEBUG:
            logr.debug(
                (
                    # "obj:{}".format(self.__class__.__name__),
                    "id:{}".format(self.acmi_id[0, 0].item()),
                    "tas:{:.3g}".format(tas[0].item()),
                    # "|Qew|:{:.3g}".format(Qew_next[0].norm().item()),
                    "pos_e:{}".format(pos_e_next[0, 0].cpu().tolist()),
                    "vel_e:{}".format(tas_next[0, 0].cpu().tolist()),
                    "mu:{:.3g}".format(mu_next[0, 0].item()),
                    "mass:{:.3g}".format(mass_next[0, 0].item()),
                )
            )
    ...
    def _f(
        ...
        Vinv = 1 / tas
        dot_tas, a_vy, a_vz = torch.chunk(a_w, 3, dim=-1)
        _P = dot_mu
        _Q = -a_vz * Vinv
        _R = a_vy * Vinv
        dot_Qew = quat_mul(Qew, 0.5 * torch.cat([_0f, _P, _Q, _R], -1))
    ...
    def png(
        self,
        target_position_e: torch.Tensor,  # (N,3)
        target_velocity_e: torch.Tensor,  # (N,3)
    ) -> torch.Tensor:
        """单目标信息->PN体轴法向、侧向过载"""
        ...
        rr = (los * los).sum(-1, keepdim=True).clip(1e-3)
        omega = torch.cross(los, dlos, dim=-1) / rr
        ad_e = self._K_PN * torch.cross(
            dlos, omega, dim=-1
