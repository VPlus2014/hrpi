...
# 2. 用 split 而不是 [...]
import math
from typing import Any, Sequence, TypeVar, Union, cast, overload
from typing_extensions import TypeAlias
import numpy as bkbn
from numpy.typing import NDArray as NDArray

cat = concatenate = bkbn.concatenate
stack = bkbn.stack
sin = bkbn.sin
cos = bkbn.cos
tan = bkbn.tan
...
broadcast_to = bkbn.broadcast_to
repeat = bkbn.repeat
tile = bkbn.tile
linspace = bkbn.linspace
logspace = bkbn.logspace
...
take = bkbn.take
full = bkbn.full
fill_diagonal = bkbn.fill_diagonal
roll = bkbn.roll
...
isnan = bkbn.isnan
isinf = bkbn.isinf
land = logical_and = bkbn.logical_and
lor = logical_or = bkbn.logical_or
lnot = logical_not = bkbn.logical_not
lxor = logical_xor = bkbn.logical_xor
eq = equal = bkbn.equal
neq = not_equal = bkbn.not_equal
minimum = bkbn.minimum
maximum = bkbn.maximum
argmax = bkbn.argmax
argmin = bkbn.argmin
...
int32 = bkbn.int32
int64 = bkbn.int64
float32 = bkbn.float32
float64 = bkbn.float64
bool_ = bkbn.bool_
bool = bool_
int_ = bkbn.intp
float_ = bkbn.float32
generic = bkbn.generic
number = bkbn.number
str_ = bkbn.str_
...
def norm_p(x, p=2, dim=-1, keepdim=True) -> Float_NDArr:
    r"""
    torch.norm 的 numpy 实现
    Args:
...
def affcmb_inv(a, b, y, eps: float = 1e-8) -> Float_NDArr:
    ...
    y_ = y - a
    _bad = absolute(m) < eps
    w = where(_bad, 0.5, y_) / where(_bad, 1, m)
    return w
...
def is_normalized(v: ndarray, eps: float = 1e-6) -> BoolNDArr:
    ...
    Returns:
        _Tensor: (...,1)
    """
    return absolute(norm_p(v) - 1) <= eps
