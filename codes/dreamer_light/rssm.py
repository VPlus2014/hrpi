...
class RSSM(nj.Module):
  ...
  deter: int = 1024  # Reduced for lightweight
  hidden: int = 512
  stoch: int = 32
  classes: int = 32
  norm: str = 'rms'
  ...
  def _observe(self, carry, tokens, action, reset, training):
    ...
    x = self.sub('obs', nn.Linear, self.hidden, **self.kw)(x)
    x = nn.act(self.act)(self.sub('obsnorm', nn.Norm, self.norm)(x))
  ...
  def _prior(self, feat):
    x = feat
    x = self.sub('prior', nn.Linear, self.hidden, **self.kw)(x)
    x = nn.act(self.act)(self.sub('priornorm', nn.Norm, self.norm)(x))
