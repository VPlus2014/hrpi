...
class BaseModelGroup(ABC):
    ...
    def __init__(
        ...
        _group_shape = np.ravel(group_shape).tolist()
        assert len(_group_shape) > 0, ("group_shape must be not empty", group_shape)
        self._group_shape = _group_shape = tuple(_group_shape)
        ...
        """Tacview Object ID, shape: (...,N,1) NDArray[intp]"""
        ...
        """Tacview 父对象 ID, shape: (...,N,1) NDArray[intp]"""
    ...
    def _ppgt_Ve2tas(self, mask: SupportedMaskType | None = None):
        """地轴系惯性速度->真空速tas"""
        mask = self.proc_to_mask(mask)
        self._tas[mask, :] = norm_p(self._vel_e[mask, :])

    def _ppgt_Vb2tas(self, mask: SupportedMaskType | None = None):
        """体轴系惯性速度->真空速tas"""
        mask = self.proc_to_mask(mask)
        self._tas[mask, :] = norm_p(self._vel_b[mask, :])
