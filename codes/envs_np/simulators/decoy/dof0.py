...
class DecoyDOF0Ball(BaseDecoy):
    ...
    def _run_ode(
        self,
        dt_s: float | mext.ndarray,  # 积分步长(不考虑掩码) unit: sec
        t_s: mext.ndarray,  # 初始时间 unit: sec
        pos_e: mext.ndarray,  # 初始位置地轴系坐标
        vel_e: mext.ndarray,  # 初始速度地轴系坐标
        mask: mext.BoolNDArr,
    ):
        ...
        rst = ode_solver(self._f, t_s, mext.cat((pos_e, vel_e), axis=-1), dt_s)
        pos_e_next, vel_e_next = mext.split_(rst, [3, 3], -1)
        if self.DEBUG:
            logr.debug(
                (
    ...
    def _f(self, t: mext.ndarray | float, X: mext.ndarray):
        """动力学"""
        p_e, v_e = mext.split_(X, [3, 3], -1)
        a_e = self._0F
        if self.use_gravity:
            a_e = self.g_e()
