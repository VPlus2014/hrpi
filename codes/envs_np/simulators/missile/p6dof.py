...
if TYPE_CHECKING:
    from .base import BaseModelGroup, BaseMissile, SupportedMaskType
# import torch
import numpy as np

from collections.abc import Sequence
from .base import BaseMissile
from ._utils import mext as mext

ndarray = mext.ndarray
BoolNDArr = mext.BoolNDArr
Float64NDArr = mext.Float64NDArr
# from .base_aircraft import BaseMissile


class MissileP6DOF(BaseMissile):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._ic_tas = mext.zeros(
            _shape + [1],
            dtype=dtype,
            # device=device,
        )
        self._ic_rpy_ew = mext.zeros(
            _shape + [3],
            dtype=dtype,
            # device=device,
        )
        self._ic_pos_e = mext.zeros(
            _shape + [3],
            dtype=dtype,
            # device=device,
        )
    ...
    def run(self, mask: SupportedMaskType | None):
        ...
        # 后处理
        super().run(msk)  # time++
        Qew_next = mext.normalize(Qew_next)
        tas_next = mext.clip(tas_next, self._Vmin, self._Vmax)  # 防止过零
    ...
    def _update_los(
        ...
        unit_id = mext.unsqueeze(unit_id, -3)  # (...,1,n,1)
        unit_id = unit_id + mext.unsqueeze(
            mext.zeros_like(self.acmi_id), -2
        )  # (...,N,n,1)
        self._all_tgt_id = unit_id  # (...,N,1,1)

        unit_pos = mext.unsqueeze(unit_pos, -3)  # (...,1,n,d)
        unit_vel = mext.unsqueeze(unit_vel, -3)
        ego_pos = mext.unsqueeze(self._pos_e, -2)  # (...,N,1,d)
        ego_vel = mext.unsqueeze(self._vel_e, -2)
        los = unit_pos - ego_pos  # (...,N,n,3)
        dlos = unit_vel - ego_vel
        self._all_los_e = los  # (...,N,n,3)
        self._all_dlos_e = dlos
        self._all_los_mask = mext.unsqueeze(mask.swapaxes(-2, -1), -1)  # (...,N,n,1)
    ...
    def _los2info(self):
        ...
        iw_e = mext.unsqueeze(
            mext.quat_rotate(self._Q_ew, self._E1F), -2
        )  # (...,N,1,d)
        los_e = self._all_los_e  # (...,N,n,3)
        dij = mext.clip(
            mext.norm_p(los_e, 2, -1, True), 1e-3, None
        )  # (...,N,n,1)
        ...
        anydet = self._candet.any(
            -2, keepdims=True
        )  # 大视场有目标 (...,N,1,1)
        anytrk = self._cantrk.any(
            -2, keepdims=True
        )  # 小视场有目标 (...,N,1,1)
        if self.DEBUG:
            anydet

        w1: BoolNDArr = mext.where(anytrk, cantrk, candet)  # 视野过滤(>where) (N,n,1)
        ...
        w2_ = mext.where(anydet, w2 / w2sum, 0)  # (N,n,1)
        all_los = self._all_los_e  # (N,n,3)
        tgt_los = (w2_ * all_los).sum(-2)  # (N,3)
        tgt_dlos = (w2_ * all_dlos).sum(-2)  # (N,3)

        Qew = self._Q_ew
        fake_los = mext.quat_rotate(Qew, self._E1F * self._det_rmax)  # (N,3)
        fake_vel = self._vel_e  # (N,3)

        _anytrk = anytrk.reshape(N, 1)  # (N,1)
        tgt_los = mext.where(_anytrk, tgt_los, fake_los)  # (N,3)
        tgt_dlos = mext.where(_anytrk, tgt_dlos, fake_vel)  # (N,3)
        ...
        dij = mext.norm_p(tgt_los, dim=-1, keepdim=True)  # (N,1)
    ...
    def set_target(
        ...
        Args:
            ...
            dst_index (_SupportedIndexType|None): 分配索引, 长度=N
        """
    ...
    def set_action(
        ...
        ny_cmd, nz_cmd = mext.unbind_keepdim(value, -1)
        # 过载限制
    ...
    def _run_ode(
        ...
        """
        ode_solver = mext.ode_rk23
        logr = self.logger

        ny, nz = mext.unbind_keepdim(action, -1)  # (...,1)

        f = lambda t, X: self._f(t, X, ny, nz)
        dt_s = dt_s * (self.is_launched() & mask[..., None])
        rst = ode_solver(f, t_s, mext.cat((pos_e, tas, Qew, mu, mass), axis=-1), dt_s)
        pos_e_next, tas_next, Qew_next, mu_next, mass_next = mext.split_(
            rst, [3, 1, 4, 1, 1], -1
        )
        if self.DEBUG:
            logr.debug(
    ...
    def _f(
        ...
        """动力学"""
        p_e, tas, Qew, mu, mass = mext.split_(X, [3, 1, 4, 1, 1], -1)
        ...
        n_w = mext.cat([nx, ny, nz], axis=-1)

        g = self._g  # 重力加速度
        a_w = g * n_w  # 过载加速度风轴分量
        if self.use_gravity:
            Qwe = mext.quat_conj(Qew)
            a_w += mext.quat_rotate(Qwe, self.g_e())

        # 旋转角速度
        dot_mu = (-self._dmu_K) * mu
        tas = mext.clip(tas, self._Vmin, self._Vmax)  # 防止过零
        Vinv = 1 / tas
        dot_tas, a_vy, a_vz = mext.unbind_keepdim(a_w, -1)
        _P = dot_mu
        _Q = -a_vz * Vinv
        _R = a_vy * Vinv
        dot_Qew = mext.quat_mul(Qew, 0.5 * mext.cat([_0f, _P, _Q, _R], axis=-1))
        dot_p_e = mext.quat_rotate(
            Qew, mext.cat([tas, _0f, _0f], axis=-1)
        )  # 惯性速度
    ...
    def png(
        ...
        rr = mext.clip(rr, 1e-6, None)  # 防止除零
        omega = mext.cross(los, dlos, axis=-1) / rr
        ad_e = self._K_PN * mext.cross(
            dlos, omega, axis=-1
        )  # required acceleration in earth frame (...,3)
        ad_b = mext.quat_rotate_inv(self.Q_ew(), ad_e)
