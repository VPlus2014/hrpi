...
from .utils.space_tf import batch_space
...
from .utils.space_tf import space2box, flatten, unflatten
...
from .utils.tacview_render import (
    ObjectState,
    AircraftAttr,
    WaypointAttr,
    get_obj_id,
)
from ..utils.tacview import ACMI_Types, TacviewRecorder, format_id
...
class NavHeadingEnv(NPSyncVecEnv):
    r"""
    ...
    def __init__(
        ...
        nz_max: float = 0.8,  # NED+Z方向最大过载, unit:G
        nz_min: float = -10.0,  # NED+Z方向最小过载, unit:G
        ...
        render_dir: PathLike | str | None = None,
        ...
        version="1.0",
        easy_mode=False,
        use_multi_reward=False,  # return reward components in info
        **kwargs,
    ):
        ...
        self._time_range = spaces.Box(
            low=0,
            high=self.max_sim_time_ms,
            shape=(1,),
            dtype=np.int64,
        )
        ...
        self.use_multi_reward = bool(use_multi_reward)
        ...
        # self._render_done = np.zeros(nenv, np.bool_)  # 轨迹是否正常结束
        ...
        self._waypoints, self._waypoint_index = self.generate_waypoints(
            nenvs=nenv, seed=None
        )
        return
    ...
    INFOKEY_REWARDS = "reward_components"
    ...
    def _make_game_v1(self):
        ...
        from .simulators.plane.p6dof import PlaneP6DOF as Plane

        # 创建飞机模型
        pln = Plane(
            sim_step_size_ms=self._sim_step_size_ms,
            group_shape=nenv,
            acmi_id=0x10086,
            acmi_name="J-10",
            call_sign="agent",
        ...
        _obs_spc[self.OBSKEY_TIME_LEFT] = self._time_range
        ...
        self._single_observation_space = spaces.Dict(_obs_spc)
        ...
        self._single_action_space = spaces.Dict(_act_spc)

        diam = float(self._region_diam)
        tc_p_dmax = diam * 0.8  # 过远
        tc_p_dmin = min(1000, diam * 1e-1)  # 抵达距离
        ...
        self._reward_fns: dict[str, BaseRewardFn] = {
            ...
            for i, rf in enumerate(
                [
                    RF_GoalFar(weight=100),
                    RF_GoalHeadingAngle(weight=1e-1),
                    # RF_GoalDistance(
                    #     use_mileage=True, use_quadratic=True, weight=1e3 / (diam**2)
                    # ), # 前期不要用 SSP
                    # RF_GoalDistance(
                    #     use_mileage=False, use_quadratic=True, weight=1e0 / (diam**2)
                    # ),
                    RF_GoalDistance(
                        use_mileage=False, use_quadratic=False, weight=1e0 / (diam)
                    ),
                    RF_GoalReach(weight=100),
                    RF_TimeCost(weight=0.1),  # 代价
                ]
            )
        }
        ...
        self._termination_fns: list[BaseTerminationFn] = [
            ...
            TC_ReachAllGoal(),
            TC_Timeout(
                self.max_sim_time_ms * 1e-3
            ),  # 如果加入这个,状态里需要引入时间维否则可能出现状态回环
        ]
    ...
    def single_observation_space(self):
        return space2box(self._single_observation_space, dtype=self.dtype)

    @cached_property
    def single_action_space(self):
        return space2box(self._single_action_space, dtype=self.dtype)
    ...
    def action_space(self):
        return batch_space(self.single_action_space, n=self.num_envs)
    ...
    # @np.no_grad()
    def reset_core(self, env_mask: EnvMaskType | None):
        ...
        if self.use_multi_reward:
            info[self.INFOKEY_REWARDS] = np.zeros(
                (nenvs_rst, len(self._reward_fns)),
                dtype=self.dtype,
            )

        obs_vec = flatten(self._single_observation_space, obs_dict)
    ...
    # @np.no_grad()
    def step_core(self, action: np.ndarray):
        info = {}
        ...
        if self.use_multi_reward:
            info[self.INFOKEY_REWARDS] = np.concatenate(rews_arr, axis=-1)

        truncated = self.sim_time_ms >= self.max_sim_time_ms  # @step, (...,1)
        terminated, term_events = self.__is_terminated()

        done = terminated | truncated  # (...,1)
        if done[i_rcd]:
            sw = self._writer
            ki = self._cur_act_step[i_rcd, ...].item()
            ei = self._cur_episode[i_rcd, ...].item()
            if sw:
                sw.add_scalar("episode_length", ki, ei)

            rew_info = []
            gtotal = 0.0
            for rfname, rf in self._reward_fns.items():
                gi = self._rew_sum[rfname][i_rcd, ...].item()
                rew_info.append(f"{rfname}: {gi:.3g}")
                if sw:
                    sw.add_scalar(f"episode_reward/{rfname}", gi, ei)
                gtotal += gi

            rew_info.append(f"total: {gtotal:.3g}")
            if sw:
                sw.add_scalar("episode_reward_total", gtotal, ei)

            if len(rew_info):
                self.logger.debug(
                    "Env{}@Ep{} terminated returns:\n{}".format(
                        i_rcd, ei, ", ".join(rew_info)
                    )
                )

        info[self.INFOKEY_OBS] = obs_dict
        info[self.INFOKEY_RET] = self._rew_sum
        info[self.INFOKEY_TERM_EVENT] = term_events
        ...
        if truncated[i_rcd]:
            done_events = deepcopy(term_events)
            done_events.append("truncated")
        else:
            done_events = term_events
        self.render_events(done_events)

        obs_out = obs_vec
        self._cur_episode += done
        return (
            deepcopy(obs_out),
            deepcopy(rew),
            deepcopy(terminated),
    ...
    def __get_obs(self, env_mask: EnvMaskType | None) -> dict[str, np.ndarray | Any]:
        ...
        if version.startswith("1."):
            ...
            obs_dict[self.OBSKEY_QEW] = Qew = pln.Q_ew()[env_mask, :]
            obs_dict[self.OBSKEY_MU] = pln.mu()[env_mask, :]
            obs_dict[self.OBSKEY_TIME_LEFT] = (
                self.max_sim_time_ms - self.sim_time_ms[env_mask, ...]
            )
