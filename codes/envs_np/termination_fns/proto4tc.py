...
if TYPE_CHECKING:
    from ..proto4venv import NPSyncVecEnv, EnvMaskType
    from ..simulators.proto4model import BaseModelGroup


class BaseTerminationFn(ABC):
    def __init__(self, env: NPSyncVecEnv | None = None, **kwargs) -> None:
        super().__init__()
        self.env = env
    ...
    @abstractmethod
    def forward(self, env: NPSyncVecEnv, unit: BaseModelGroup, **kwargs) -> np.ndarray:
        pass

    def __call__(self, env: NPSyncVecEnv, unit: BaseModelGroup, **kwargs) -> np.ndarray:
        try:
