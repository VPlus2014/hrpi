...
from agents.policy.proto4policy import BaseNNPolicy
from replay_buffer.replaybuffer import ReplayBuffer
from agents.replay_buffer.proto4data import RolloutBatchProtocol
from agents.replay_buffer.np_trajbuffer import RETrajReplayBuffer

from envs_pt.utils.space import normalize as normalize_th


class PPOContinuous(BaseNNPolicy):
    ...
    def __init__(
        ...
        logr: logging.Logger = BaseNNPolicy.logger,
    ):
        self.logger = logr
        super().__init__(
            name,
        ...
        if use_re_buffer:
            self.replay_buffer = RETrajReplayBuffer(
                ...
                size_stack_in=num_envs,
                float_dtype=np_float,
                act_dtype=np_action_dtype,
                logger=logr.getChild("replay_buffer"),
            )
        else:
    ...
    def learn(self, global_step: int):
        logr = self.logger
        ...
        # 归一化
        obs = normalize_th(obs, self.observation_min, self.observation_max)
